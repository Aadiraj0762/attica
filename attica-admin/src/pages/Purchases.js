...
const Coupons = () => {
  ...
  const deleteApi = async (id) => {
    try {
      const response = await fetch(`http://localhost:5055/api/purchases/purchases/${id}`, {
        method: 'DELETE',
        headers: {
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:5055/api/purchases/purchases') // Update the API endpoint to the correct URL
